...
| **SSH** | Secure Shell - Targets SSH services to brute-force credentials, commonly used for secure remote login to systems | `hydra -l root -P /path/to/password_list.txt ssh://192.168.1.100` |
...
#### Medusa Module Examples

| Service/Protocol | Description | Example Command |
|------------------|-------------|-----------------|
| **SSH** | Secure Shell - Brute force SSH login for the admin user | `medusa -h 192.168.1.100 -u admin -P passwords.txt -M ssh` |
...
| **SSH (First Valid)** | Secure Shell - Stop after the first valid SSH login is found | `medusa -h 192.168.1.100 -u admin -P passwords.txt -M ssh -f` |
...
CUPP creates personalized password wordlists based on gathered intelligence.
...
*This cheat sheet is for educational and authorized security testing purposes only. Always ensure you have proper authorization before conducting any security testing.*
