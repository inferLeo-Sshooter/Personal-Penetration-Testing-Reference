...
Kerberos is a ticket-based authentication system that avoids sharing account passwords with every service. Instead, it stores tickets locally and presents service-specific tickets.

Key components:
- **Ticket Granting Ticket (TGT)**: First ticket obtained, allows requesting additional Kerberos tickets or TGS
- **Ticket Granting Service (TGS)**: Service-specific tickets that verify user identity

Authentication process:
1. User requests TGT by encrypting current timestamp with their password hash
2. Domain controller validates identity (can decrypt timestamp using known password hash) and sends TGT
3. User no longer needs password for future requests
4. To access services (e.g., MSSQL), user requests TGS from Key Distribution Center (KDC) using TGT
...
Traditional `Pass the Hash` (`PtH`) reuses NTLM password hashes without touching Kerberos. `Pass the Key` aka. `OverPass the Hash` converts a hash/key (rc4_hmac, aes256_cts_hmac_sha1, etc.) for domain users into full `Ticket Granting Tickets` (`TGT`). Developed by Benjamin Delpy and Skip Duckwall in "Abusing Microsoft Kerberos - Sorry you guys don't get it," and adapted by Will Schroeder for the Rubeus tool.

To forge tickets, obtain the user's hash via Mimikatz using `sekurlsa::ekeys`, which enumerates all Kerberos package key types.
...
PowerShell Remoting enables running scripts/commands on remote computers, commonly used by administrators for network management. It creates HTTP (TCP/5985) and HTTPS (TCP/5986) listeners.

Requirements for PowerShell Remoting sessions:
- Administrative permissions, OR
- Remote Management Users group membership, OR  
- Explicit PowerShell Remoting permissions in session configuration

Non-administrative users in the Remote Management Users group can still use PowerShell Remoting to connect and execute commands on remote computers.

## Mimikatz - PowerShell Remoting with Pass the Ticket

To use PowerShell Remoting with Pass the Ticket: open `cmd.exe`, run `mimikatz.exe`, import the ticket using `kerberos::ptt`. From the same `cmd.exe` session, launch PowerShell and use `Enter-PSSession` to connect to the target machine.
