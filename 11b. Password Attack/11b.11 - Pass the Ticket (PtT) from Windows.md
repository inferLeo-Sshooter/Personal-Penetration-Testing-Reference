...
---
If target is Windows machine or related to Windows by joining its Domain, AND Windows does support Kerberos Authentication --> Pass-The-Ticket-able.

These 2 tools: Mimikatz and Rubeus, you can perform the attacks without relying on each other.

1. Harvesting Kerberos tickets from Windows

	`Collect ticket with mimikatz or Rubeus (if admin --> dump all tickets, else just current user)`

2. Pass the Key aka. OverPass the Hash (converts a hash/key for domain users into full TGT)

	`Extract Kerberos keys with mimikatz --> perform the OverPass the Hash with mimikatz`
...
```
after collecting tickets, we can:

	Option 1: Use Rubeus to retrieve tickets and submit it to the current session in 1 line

	Option 2: Use Rubeus to import the ticket into the current session using the .kirbi file from the disk

	Option 3: Use the Base64 output from Rubeus or convert a .kirbi to Base64 to perform the Pass the Ticket attack

	Option 4: perform the Pass the Ticket attack using the Mimikatz module kerberos::ptt and the .kirbi file that contains the ticket we want to import. (use the Mimikatz module misc to launch a new command prompt window with the imported ticket)
```

4. Pass The Ticket with PowerShell Remoting (Windows)
```
	  MS01 -- do PTT --> DC01
	   |		    	  |
	   |		    	  |
	U R here		Try get here
```
```	
	Option 1 (use mimikatz): open cmd.exe --> run mimikatz.exe --> import the ticket using kerberos::ptt. From the same cmd.exe session --> launch PowerShell --> use Enter-PSSession to connect to the target machine.

	Option 2 (use rubeus): Create a sacrificial process with Rubeus --> will open a new cmd window --> From that window execute Rubeus to request a new TGT --> import the ticket into our current session --> connect to the DC using PowerShell Remoting.
```
---
