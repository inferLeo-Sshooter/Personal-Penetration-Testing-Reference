...
PKINIT (`Public Key Cryptography for Initial Authentication`) is a Kerberos extension enabling public key cryptography during initial authentication, typically for smart card logons storing private keys. `Pass-the-Certificate` uses X.509 certificates to obtain `Ticket Granting Tickets (TGTs)`, primarily in AD CS attacks and Shadow Credential attacks.
