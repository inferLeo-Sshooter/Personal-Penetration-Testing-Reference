...
## 8. Note
```
take notice on requests
correct data forms
hidden data forms in requests

take note on request of script files
take notice on the result of POST request
even in POST request body, still url encode spec chars. When found new parameter, test again with spec chars. And finally fuzz with Server-side variable names.
```
