...
- "Living off the land" was coined by Christopher Campbell @obscuresec & Matt Graeber @mattifestation at DerbyCon 3. LOLBins (Living off the Land binaries) originated from a Twitter discussion about naming binaries attackers can use beyond their intended purpose. Two websites aggregate this information:
* LOLBAS Project for Windows Binaries  
* GTFOBins for Linux Binaries

- Living off the Land binaries enable functions like download, upload, command execution, file read/write, and bypasses. This section covers using LOLBAS and GTFOBins projects with download/upload examples for Windows & Linux systems.
...
- Let's use `OpenSSL` for example. It's frequently installed and often included in other software distributions, with sysadmins using it to generate security certificates, among other tasks. OpenSSL can be used to send files "nc style."
