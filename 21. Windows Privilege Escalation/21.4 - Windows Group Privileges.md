...
│   ├── 1. Use script to get PoC
│   ├── 2. Attack Domain Controller
│   │   ├── Copy NTDS.dit
│   │   └── Dump hashes
│   ├── 3. Back up SAM and SYSTEM registry hives
│   ├── 4. Copy files with built-in Robocopy utility
│   └── Note: Explicit deny entries for current user or their group will block access,
│           even with FILE_FLAG_BACKUP_SEMANTICS specified.
│
├── II. Event Log Readers
│   ├── 1. Find creds by querying Windows events
│   │   ├── `wevtutil` (CLI)
│   │   └── `Get-WinEvent` (PowerShell)
│   ├── Note: Searching Security event log with Get-WinEvent requires admin access
│   │         or adjusted perms on HKLM\System\CurrentControlSet\Services\Eventlog\Security.
│   │         Event Log Readers membership alone is not sufficient.
│   ├── Note: Cmdlet can run as another user via `-Credential` parameter.
│   └── Note: PowerShell Operational log may contain creds if script block or module
│             logging is enabled — accessible to unprivileged users.
│
├── III. DnsAdmins
│   ├── 1. Privilege Escalation on DC (DNS runs as NT AUTHORITY\SYSTEM)
│   │   ├── Generate malicious DLL and upload to target
│   │   ├── Load DLL as DnsAdmins member (must specify full path)
│   │   ├── DLL loads on next DNS service start
│   │   ├── Restart DNS service
│   │   └── ⚠ Note: Highly destructive
│   ├── 2. Cleaning Up (requires elevated console with local/domain admin)
│   │   ├── Confirm registry key was added
│   │   ├── Delete registry key pointing to custom DLL
│   │   ├── Start DNS service again
│   │   └── Check DNS service status
│   ├── 3. Using mimilib.dll
│   │   └── Modify kdns.c to execute reverse shell or custom command
│   └── 4. Creating a WPAD Record
│       ├── Disable Global Query Block List
│       ├── Create WPAD record as DnsAdmins member
│       ├── All default-WPAD machines proxy traffic through attacker
│       └── Capture/crack hashes or SMBRelay via Responder/Inveigh
│
├── IV. Print Operators
│   ├── Grants SeLoadDriverPrivilege
│   ├── 1. Check privs — if SeLoadDriverPrivilege missing from unelevated context,
│   │      bypass UAC first (`whoami /priv`)
│   ├── 2. Exploit with GUI — via Capcom.sys (any user → SYSTEM shellcode)
│   │   ├── Download and compile script tool
│   │   ├── Add reference to driver
│   │   └── Use ExploitCapcom tool to escalate
│   ├── 3. Exploit without GUI
│   │   ├── Modify ExploitCapcom.cpp before compiling
│   │   ├── Set up listener from msfvenom payload
│   │   └── Fallback: bind shell or exec/add user payload
│   ├── 4. Auto-pwn with EopLoadDriver
│   ├── 5. Clean-up — delete registry key added during exploit
│   └── Note: SeLoadDriverPrivilege not exploitable on Windows 10 v1803+
│             (HKEY_CURRENT_USER registry references no longer allowed)
│
└── V. Server Operators
    ├── Grants SeBackupPrivilege, SeRestorePrivilege, and local service control
    └── Exploit
        ├── Query AppReadiness Service — confirm it starts as SYSTEM
        ├── Check service perms with PsService
        │   └── Server Operators have SERVICE_ALL_ACCESS → full control
        ├── Modify service binary path to add current user to local admins
        ├── Start service and confirm local admin group membership
        ├── Confirm local admin access on Domain Controller
        └── Retrieve NTLM password hashes from DC
```
---
