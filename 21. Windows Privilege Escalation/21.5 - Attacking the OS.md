...
├── I. UAC
│   └── 1. Bypass
│       ├── Confirm if UAC is enabled and at what level
│       ├── Check Windows version (bypasses differ per build)
│       └── Compare findings with the UACME project
│
├── II. Weak Permissions
│   ├── 1. Permissive File System ACLs
│   │   ├── Run SharpUp — check service binaries with weak ACLs
│   │   ├── Check permissions with `icacls`
│   │   └── Replace service binary with shell
│   ├── 2. Weak Service Permissions
│   │   ├── Review SharpUp output
│   │   ├── Check permissions with `AccessChk`
│   │   ├── Change the service binary path
│   │   ├── Restart the service
│   │   └── Clean-up — reset binary path back to original
│   ├── 3. Unquoted Service Path
│   │   ├── Enumerate unquoted service paths
│   │   └── Write malicious executable to an earlier path in the search order
...
│       ├── Use `wmic` to list programs that run at startup
│       ├── Check for write permissions on the binary
│       └── If writable — escalate when that user next logs in
│
├── III. Kernel Exploits
│   └── 1. Enumerate Missing Patches
│       ├── Examine installed updates via commands
│       └── View installed updates with WMI
│
└── IV. Vulnerable Services
    ├── Note: Even well-patched systems can be exploited via user-installed
    │         software or vulnerable third-party applications
    └── Steps
